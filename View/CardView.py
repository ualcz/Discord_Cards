...
class CollectionPaginationView(discord.ui.View):
    ...
    async def update_message(self, interaction: discord.Interaction):
        ...
        if not cards:
            discord.Embed(title=f"Nenhum card disponível na coleção '{collection['name']}'.", color=color['coletion_color'])
            await interaction.response.edit_message(embed=embed, view=self)
            return

        card = cards[self.current_card_index]
        
        embed = discord.Embed(title=f"Card na Coleção '{collection['name']}'\n({self.current_card_index + 1}/{len(cards)})", color=color[card['rarity']])
        embed.add_field(name="Nome", value=card['name'], inline=False)
        embed.add_field(name="Raridade", value=rarity_to_emoji.get(card['rarity']), inline=False)
    ...
    async def show_card_info(self, interaction: discord.Interaction):
        ...
        collection = await interaction.client.database.get_collection(card['collection_id'])
        count_users = await interaction.client.database.count_users_with_card(card['id'])

        embed = discord.Embed(title=f"Info: {card['name']}", color=color[card['rarity']])
    ...
    async def get_initial_embed(self, interaction: discord.Interaction):
        ...
        cards = await interaction.client.database.get_cards_by_collection(collection_id)
        if not cards:
            return discord.Embed(title=f"Nenhum card disponível na coleção '{collection['name']}'.", color=color['coletion_color'])

        card = cards[self.current_card_index]

        embed = discord.Embed(title=f"Card na Coleção '{collection['name']}\n({self.current_card_index + 1}/{len(card)})'", color=color[card['rarity']])
        embed.add_field(name="Nome", value=card['name'], inline=False)
        embed.add_field(name="Raridade", value=rarity_to_emoji.get(card['rarity']), inline=False)
        embed.set_thumbnail(url=card['image_url'])
...
class UserCollectionPaginationView(discord.ui.View):
    ...
    async def update_message(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"Card '{collection['name']}' by {self.user.name} \n({self.current_card_index + 1}/{len(all_cards)})", color=color[card['rarity']])
        embed.set_thumbnail(url=card['image_url'])
        embed.add_field(name="Nome", value=card['name'], inline=False)
        embed.add_field(name="Raridade", value=rarity_to_emoji.get(card['rarity']), inline=False)
...
class UserCollectionPaginationView(discord.ui.View):
    ...
    async def update_message(self, interaction: discord.Interaction):
        ...
        # Calcular quantos cards faltam para o usuário completar a coleção
        total_cards = len(all_cards)
        cards_faltando = total_cards - len(user_cards)
        embed.add_field(name="Faltam", value=f"{cards_faltando} cards para completar a coleção", inline=False)
